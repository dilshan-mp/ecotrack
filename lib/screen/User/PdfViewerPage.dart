...
class PdfViewerPage extends StatefulWidget {
  ...
  const PdfViewerPage({Key? key, required this.reportUrl}) : super(key: key);

  @override
  _ReportViewerState createState() => _ReportViewerState();
}

class _ReportViewerState extends State<PdfViewerPage> {
  ...
  void initState() {
    super.initState();
    _downloadAndOpenFile(widget.reportUrl);
  }

  Future<void> _downloadAndOpenFile(String url) async {
    final response = await http.get(Uri.parse(url));
    final documentDirectory = await getApplicationDocumentsDirectory();
    final file = File('${documentDirectory.path}/report.pdf');
